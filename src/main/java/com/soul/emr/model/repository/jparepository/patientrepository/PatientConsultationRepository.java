...
{
	@Transactional
	@Query("SELECT o FROM PatientConsultationDB o WHERE o.employeeInfoDB.doctorCode = :doctorCode AND o.type = :patientType AND (:date IS NULL OR o.encounterDate = :date) AND o.consultationStatus = :consultationStatus")
	...
	@Transactional
	@Query("SELECT COUNT(p) FROM PatientConsultationDB p WHERE (:type IS NULL OR p.type = :type) AND p.employeeInfoDB.doctorCode = :doctorCode")
	...
	@Transactional
	@Query("SELECT p.patientDetailDB.gender, p.encounterDate, COUNT(p) FROM PatientConsultationDB p WHERE (:type IS NULL OR p.type = :type) AND p.employeeInfoDB.doctorCode = :doctorCode AND p.encounterDate >= :currentDate AND p.encounterDate < :currentDate GROUP BY p.patientDetailDB.gender, p.encounterDate")
	...
	@Transactional
	@Query("SELECT COUNT(p) FROM PatientConsultationDB p WHERE p.consultationType IN :consultationTypes AND p.employeeInfoDB.doctorCode = :doctorCode AND p.encounterDate = :currentDate")
