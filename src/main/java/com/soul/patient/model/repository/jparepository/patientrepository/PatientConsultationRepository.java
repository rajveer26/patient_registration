...
public interface PatientConsultationRepository extends JpaRepository<PatientConsultationDB, Long>
{
	...
	@Transactional
	@Query("SELECT COUNT(p) FROM PatientConsultationDB p WHERE (:type IS NULL OR p.type = :type) AND p.doctorCode.doctorCode = :doctorCode")
	...
	@Transactional
	@Query("SELECT p.patientDetailDB.gender, p.encounterDate, COUNT(p) FROM PatientConsultationDB p WHERE (:type IS NULL OR p.type = :type) AND p.doctorCode.doctorCode = :doctorCode AND p.encounterDate >= :currentDate AND p.encounterDate < :currentDate GROUP BY p.patientDetailDB.gender, p.encounterDate")
	...
	@Transactional
	@Query("SELECT COUNT(p) FROM PatientConsultationDB p WHERE p.consultationType IN :consultationTypes AND p.doctorCode.doctorCode = :doctorCode AND p.encounterDate = :currentDate")
